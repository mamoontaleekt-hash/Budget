...
try {
  ...
  const pwa = await evaluate(`navigator.serviceWorker.ready.then(async r=>({active:!!r.active,controlled:!!navigator.serviceWorker.controller,keys:await caches.keys(),comparison:!!(await caches.match('./comparison-model.js?v=20260908-phase7')),category:!!(await caches.match('./category-analytics-model.js?v=20260908-phase8')),report:!!(await caches.match('./report-enhancements.js?v=20260909-ux5')),stale:!!(await caches.match('./report-enhancements.js?v=20260908-phase8'))}))`);
  assert.equal(pwa.active, true); assert.equal(pwa.controlled, true); assert.ok(pwa.keys.includes("pfm-pwa-v20")); assert.equal(pwa.comparison, true); assert.equal(pwa.category, true); assert.equal(pwa.report, true); assert.equal(pwa.stale, false);
