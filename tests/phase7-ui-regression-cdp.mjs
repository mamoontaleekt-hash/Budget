...
try {
  ...
  assert.equal(pwa.active, true); assert.equal(pwa.controlled, true); assert.ok(pwa.keys.includes("pfm-pwa-v17")); assert.equal(pwa.comparison, true); assert.equal(pwa.category, true); assert.equal(pwa.report, true); assert.equal(pwa.stale, false);
