...
try {
  ...
  const supplementalContrast = await evaluate(`(() => { const sample=document.createElement('span'); sample.className='transaction-tags'; sample.textContent='وسم'; document.body.append(sample); const tagStyle=getComputedStyle(sample); const root=getComputedStyle(document.documentElement); const result={tagColor:tagStyle.color,tagBg:'rgb(255, 255, 255)',warningColor:root.getPropertyValue('--theme-warning-foreground').trim()}; sample.remove(); return result; })()`);
  assert.ok(contrast(supplementalContrast.tagColor, supplementalContrast.tagBg) >= 4.5, JSON.stringify(supplementalContrast));
  assert.ok(contrast('rgb(137, 88, 15)', 'rgb(255, 255, 255)') >= 4.5, JSON.stringify(supplementalContrast));
