...
try {
  ...
  assert.equal(ui.selected, "grocery"); assert.match(ui.options, /غير مصنف/); assert.match(ui.options, /تصنيف غير موجود \(ex_removed\)/); assert.match(ui.kpis, /150/); assert.match(ui.kpis, /600/); assert.equal(ui.trendRows, 12); assert.match(ui.trend, /تشرين الثاني/); assert.match(ui.trend, /0/); assert.match(ui.overview, /غير مصنف/); assert.match(ui.overview, /تصنيف غير موجود/); assert.equal(ui.buttons, 0); assert.match(ui.explorer, /300/); assert.doesNotMatch(ui.explorer, /9,999,999/); assert.doesNotMatch(ui.insights, /يجب|تبالغ|غير صحي|الأسعار/);
